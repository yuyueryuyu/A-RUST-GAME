...
pub fn notice_system(time: Res<Time>, mut query: Query<(&mut Notice, &Animator), With<FlyingEyes>>) {
...
pub struct FlyingEyesBehaviourPlugin<S: States> {
    pub state: S
}

impl<S: States> Plugin for FlyingEyesBehaviourPlugin<S> {
